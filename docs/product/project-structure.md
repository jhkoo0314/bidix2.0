...
│
├── utils/                    # 🛠️ 유틸리티 함수
│   ├── number.ts             # 숫자 관련 유틸리티 (roundToK, formatCurrency 등)
│   ├── outcome.ts            # 입찰 결과 포맷팅 (formatOutcome)
│   └── score.ts              # 점수 등급 변환 (mapScoreToGrade)
│
...
- `simulationservice.ts`: 시뮬레이션 서비스
  ...
  - auctionEngine 실행
  - serialization
- `__tests__/simulationservice.test.ts`: 시뮬레이션 서비스 테스트
...
- `number.ts`: 숫자 관련 유틸리티
  ...
  - `seedBasedRandom(seed: string)`: 시드 기반 랜덤 값 생성
  - `seedBasedRandomInt(seed, min, max)`: 시드 기반 정수 난수 생성
  - `formatPercentage(value, decimals)`: 백분율 포맷팅
  - `formatCurrency(value)`: 금액 포맷팅
- `outcome.ts`: 입찰 결과 포맷팅
  - `formatOutcome(outcome)`: 입찰 결과를 UI 표시용으로 포맷팅 (라벨, 아이콘, 색상)
- `score.ts`: 점수 등급 변환
  - `mapScoreToGrade(score)`: 점수를 등급(S/A/B/C/D)으로 변환

**특징**:

- ✅ **도메인 특화**: 비즈니스 로직에 특화된 유틸리티
- ✅ **순수 함수**: 부수 효과 없음
- ✅ **UI 지원**: UI 표시용 포맷팅 함수 포함
